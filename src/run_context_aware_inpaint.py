...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The prompt is used to generate results",
    )
    parser.add_argument(
        "--class_prompt",
        type=str,
...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        example = {}
        path = str(self.instance_images_path[index % self.num_instance_images])
        if "merged" in path and random.random() < 0.95:
            path.replace("merged", "source")
        ...
        if "merged" in key_name and False:
            ...
            example["mask"] = random_mask(instance_image.size, 1, False)
...
def main():
    ...
    # Image Encoder of subject images
    ...
    text_encoder = pipeline.text_encoder.to(torch.float32)
    vae = pipeline.vae
    unet = pipeline.unet.to(torch.float32)
    ...
    def collate_fn(examples):
        ...
        masked_images = []
        # load mask here
        # print("len_examples:", len(examples)) # 1
        for i, example in enumerate(examples):
            ...
            # pil_image.save("images/tmp/pil_image.jpg")
            # mask.save("images/tmp/mask.jpg")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                # print("masked_latents.shape:", masked_latents.shape)
                ...
                if ip_adapter_image is not None:
                    image_embeds = pipeline.prepare_ip_adapter_image_embeds(
                        ...
                        num_images_per_prompt = args.train_batch_size * num_images_per_prompt,
    ...
    if accelerator.is_main_process:
        # pipeline_component = StableDiffusionPipeline.from_pretrained(
        #     args.pretrained_model_name_or_path,
        #     unet=accelerator.unwrap_model(unet),
        #     text_encoder=accelerator.unwrap_model(text_encoder),
        #     image_encoder=accelerator.unwrap_model(image_encoder),
        # )
        # pipeline_component.save_pretrained(args.output_dir)
