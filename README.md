...
Before running the model, you need to check whether the image data is ready by following the steps below. It is recommended to put the images and scripts used in one experiment into an isolate folder to avoid interfering with other codes, and `exp` folder is such an example.
